...
async function snippetLogicToLAPSimple(snippets, outDir=null) {

  if (!fs.existsSync(`${outDir}/toLawScript`)) {
    fs.mkdirSync(`${outDir}/toLawScript`);
  }

  const logic = [];
  for (let [index, snippet] of snippets.entries()) {
    const pageCachePath = `${outDir}/toLawScript/snippet-${(index + "").padStart(5, '0')}.ls`;
    ...
    console.log(`Snippet to snippetLogicToLAPSimple, snippet ${index + 1} / ${snippets.length}`);
...
`},
  ...
  let LawScript = await prompt(samples);
    console.log(LawScript);
    fs.writeFileSync(pageCachePath, LawScript.answer);
    logic.push(LawScript.answer);
...
module.exports = snippetLogicToLAPSimple;
